...
use tokio::{io::AsyncBufReadExt, select, time};
...
pub struct ChatApp {
    ...
    input: String,
    config: Config,
    shared_secrets: HashMap<PeerId, SharedSecret>,
...
impl ChatApp {
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let gossipsub_config = gossipsub::ConfigBuilder::default()
            ...
            .message_id_fn(gossipsub::fast_message_id_fn)
            .build()
            .expect("Valid config");
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        loop {
            select! {
                event = event::poll(Duration::from_millis(0)) => {
                    if event? {
    ...
    async fn handle_key_exchange(&mut self, key_exchange: KeyExchange, from_peer: PeerId) -> Result<()> {
        ...
        let shared_secret = self.my_secret.diffie_hellman(&peer_public);
    ...
    async fn send_message(&mut self) -> Result<()> {
        ...
        for (peer_id, shared_secret) in &self.shared_secrets {
    ...
    fn encrypt_message(&self, content: &str, shared_secret: &SharedSecret) -> Option<(Vec<u8>, Vec<u8>)> {
